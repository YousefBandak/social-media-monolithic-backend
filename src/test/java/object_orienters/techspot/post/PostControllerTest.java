...
//    @Test
//    public void testGetTimelinePosts_Success() throws Exception {
//        EntityModel<Post> entityModel = EntityModel.of(post); // Wrap in HATEOAS entity model.
//        given(postService.getPosts(profile.getUsername()).willReturn(List.of(post)));
...
//    }
    ...
    public void testAddMediaPost() throws Exception {
        ...
        given(postService.addPosts("husam_ramoni",mockFile,"Post husam",Privacy.PUBLIC)).willReturn(post);
        ...
        mockMvc.perform(multipart("/profiles/{username}/posts", "husam_ramoni")
                ...
                .andExpect(jsonPath("$.textData").value(post.getTextData()))
                .andExpect(jsonPath("$.privacy").value(post.getPrivacy().toString()));

        // Verify service interaction
        verify(postService).addPosts("husam_ramoni",mockFile,"Post husam",Privacy.PUBLIC);
        verify(assembler).toModel(any(Post.class));
    ...
    public void testEditPost_Success() throws Exception {
        ...
        given(postService.editPost(username, postId, editedPost)).willReturn(editedPost);
        ...
        // Verify service interaction
        verify(postService).editPost(username, postId, editedPost);
        verify(assembler).toModel(editedPost);
        post = postService.editPost(username, postId, editedPost);
        System.out.println(postService.editPost(username, postId, editedPost));
        System.out.println(post.getTextData());
